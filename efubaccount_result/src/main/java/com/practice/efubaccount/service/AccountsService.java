...
public class AccountsService {
    ...
    // 회원 단건 조회 GET account/{accountId}
    public AccountResponseDto getAccount(Long accountId) {
        Account account = accountsRepository.findByAccountId(accountId).orElseThrow(() -> new IllegalArgumentException("해당 계정을 찾을 수 없습니다."));
    ...
    public AccountResponseDto updateAccount(Long accountId, BioUpdateRequestDto requestDto) {
        Account account = accountsRepository.findByAccountId(accountId).orElseThrow(()->new IllegalArgumentException("해당 회원을 찾을 수 없습니다."));
        account.updateBio(requestDto.getBio());
        Account updqtedAccount = accountsRepository.save(account);
        return AccountResponseDto.from(updqtedAccount);
    }

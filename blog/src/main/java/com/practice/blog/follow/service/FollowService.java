...
public class FollowService {
    ...
    // 팔로우 추가
    public FollowStatusResponseDto addFollow(Long accountId, FollowRequestDto requestDto){
        Account follower = accountService.findByAccountId(accountId);
        Account following = accountService.findByAccountId(requestDto.getFollowingId());
        if(followRepository.existsByFollowerAndFollowing(follower, following)){
            throw new BlogException(ExceptionCode.ALREADY_FOLLOWED);
        }
        followRepository.save(requestDto.toEntity(follower, following));
